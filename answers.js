...
there is going to be any rain in the next 24 hours. 
The method you use should return `true` or `false`.*/

function isItGoingToRain() {
    var twentyFour= dataset.hourly.data.slice(0, 23);
        
    return twentyFour.reduce(function(acc, currentHour) {
       if(currentHour.icon === "rain") {
           acc = true;
       }
        return acc;
    }, false);
...
function isItGoingToRainEigth() {
    return dataset.hourly.data.slice(0, 7).reduce(function(acc, currentHour) {
       if(currentHour.icon === "rain") {
           acc = true;
       } 
...
console.log(isItGoingToRainEigth());

/*## Just give me the temperature
Using the appropriate array method, start with the hourly data and return an 
array of **only** the temperatures. Your returned array should have the same number of
elements as the original array.

Example output: `[72.9, 70.5, ...]`*/

function onlyTemperatures() {
    return dataset.hourly.data.map(function(object){
        return object.temperature;
    });
}

console.log(onlyTemperatures());

/*## When is it going to rain?
Using a chain of two array methods, start with the hourly data and return an array of 
`Date` objects saying when it will rain. To do this, you'll have to:

1. Eliminate any item where it is not raining.
2. Extract the time from the remaining items and create a `Date` object*/

function whenRain() {
    return dataset.hourly.data.filter(function(object){
      if  (object.icon ==="rain") {
          return true;
      }
      else {
        ...
        return new Date(object.time * 1000);
    })
}

console.log(whenRain());

/*## Is it going to be sunny all week?
Using the appropriate array method, start with the **daily** data and
figure out whether it will be sunny **every day** of the week.
Use the `summary` and check if it contains the word `"sun"` using your preferred method.*/

function sunnyEveryDay() {
    
    return dataset.daily.data.every(function(object) {
        if(object.summary === "sun") {
            return true;
        }
    });
}

console.log(sunnyEveryDay());

/*## How hot will it be today?
Using the appropriate array method, start with the hourly data and figure out
what will be **the hottest** temperature today. 
As a hint, notice that you will need to visit each item in turn,
but also **keep track of a separate value**.*/

function hottest() {
    return dataset.hourly.data.reduce(function(acc, object) {
        if( object.temperature > acc) {
            acc = object.temperature;
        }
        return acc;
    }, 0)
}
...
Using the appropriate array method, start with the **daily** data and
find out how many times each icon -- representing the weather condition for that day -- appears.
...
function dailyIcon() {
    return dataset.daily.data.reduce(function(acc, object) {
        ...
        if(object.icon === "cloudy"){
            ...
            acc["partly-cloudy-night"] ++; 
        }
        else if(object.icon === "cloudy"){
            acc["sunny"] ++; 
        }
        else if(object.icon === "partly-cloudy-day"){
            acc["partly-cloudy-day"] ++; 
        }
        return acc;
    }, {"rain":0, "cloudy":0, "partly-cloudy-night":0, "sunny":0, "partly-cloudy-day":0});
}
...
```javascript
data*/
function fourMethods() {
    return data.filter(function(object) {
        if (object.email.slice(-4) === ".biz") {
            return true;
        }
        else {
            return false;
    ...
    .map(function(object1) {
        return object1.address.city;
    })
    ...
    .forEach(function(object3) {
        console.log(object3);
    
    });
}

console.log(fourMethods());